This file explains the use of each of the functions that can be found in the code inside this folder. The functions follow a hierarchical structure that ends with visualize_all_results().Therefore by running the final  11 lines, starting  with 'methodlist = ["No Preproc", "PCA", "RESAMPLING", "L1"]' one can run all results as the functions will trigger each other. In those lines, the path to the data needs to be replaced by the location where the data is saved, and in lines 65-67, the local paths to the model files need to be replaced. Unfortunately, the model files couldn't be uploaded to this git project due to size limitations. They can however be accessed and downloaded through the following URLS: https://huggingface.co/microsoft/codebert-base, https://huggingface.co/claudios/VulBERTa-MLP-Devign and https://github.com/SySeVR/SySeVR. 
The file dependencies.txt will contain all dependencies that have to  be installed in order for the code to run smoothly. The data can be found here: https://github.com/ZeoVan/MSR_20_Code_vulnerability_CSV_Dataset
...
- finetune_in_memory_nn:
	...
	What this function does is fine-tune the model based on the data that is available, and return a fine-tuned model to be used for feature extraction

- return_features_nn:
	Takes in the name of the NN architecture to be used, as well as the data from which to extract these features.
	What this function does is read in the source code, apply the neural network architecture of choice and return the last layer of the neural network as described in the thesis. These features will then serve as input to the pre-processing methods when applied. This function calls on finetune_in_memory_nn to return fine-tuned models that are then used to extract features from
...
- combine_results_base_multiple_runs:
	...
	This function runs the combine_results_base function multiple times and averages over the results to iron out any randomness in the models
...
-combine_results_nn_multiple_runs:
	...
	This function runs the combine_results_nn function multiple times and averages over the results to iron out any randomness in the models

- visualize_all_results:
	Takes in a base and an NN  model list, a list off preprocesing methods, and data to be fed to the base and NN models, as well as an output folder to store the results
